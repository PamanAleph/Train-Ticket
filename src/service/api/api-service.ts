...
export const getOrderByUser = async ({
  ...
  try {
    const response = await axios.get(`${apiBaseUrl}/orders/user/${userId}`);
    return response.data.Orders;
  } catch (error) {
    return error;
  }
...
export const createPaymentCode = async (paymentForm: PaymentForm) => {
  try {
...
export const userPayment = async (paymentForm: PaymentPaid) => {
  try {
    const response = await axios.post(
